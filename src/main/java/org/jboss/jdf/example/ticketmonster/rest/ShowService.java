 ...
 */
@Stateless
public class ShowService extends BaseEntityService<ShowData> {

    public ShowService() {
        super(ShowData.class);
    }
    ...
    public ShowData getShowByPerformance(@PathParam("performanceId") Long performanceId) {
        Query query = getEntityManager().createQuery("select s from ShowData s where exists(select p from Performance p where p.show = s and p.id = :performanceId)");
        query.setParameter("performanceId", performanceId);
        return (ShowData) query.getSingleResult();
    }
}
